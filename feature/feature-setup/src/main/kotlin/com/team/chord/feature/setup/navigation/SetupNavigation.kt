...
// Add menu graph route constants (standalone menu addition from menu list)
const val ADD_MENU_GRAPH_ROUTE = "add_menu_graph"
private const val ADD_MENU_SEARCH_ROUTE = "add_menu_search"
private const val ADD_MENU_DETAIL_ROUTE = "add_menu_detail"
private const val ADD_INGREDIENT_INPUT_ROUTE = "add_ingredient_input"
private const val ADD_MENU_CONFIRM_ROUTE = "add_menu_confirm"
...
// Add menu graph route patterns
private const val ADD_MENU_DETAIL_ROUTE_PATTERN =
    "$ADD_MENU_DETAIL_ROUTE/{$ARG_MENU_NAME}/{$ARG_IS_TEMPLATE_APPLIED}?$ARG_TEMPLATE_PRICE={$ARG_TEMPLATE_PRICE}"
private const val ADD_INGREDIENT_INPUT_ROUTE_PATTERN =
    "$ADD_INGREDIENT_INPUT_ROUTE/{$ARG_MENU_NAME}/{$ARG_IS_TEMPLATE_APPLIED}?$ARG_TEMPLATE_ID={$ARG_TEMPLATE_ID}"

/**
 * Navigate to the add menu graph (standalone menu addition from menu list).
 */
fun NavController.navigateToAddMenuGraph(navOptions: NavOptions? = null) {
    navigate(ADD_MENU_GRAPH_ROUTE, navOptions)
}

/**
 * Standalone menu addition graph reusing setup screen composables.
 *
 * Navigation flow:
 * MenuSearchScreen -> MenuDetailScreen -> IngredientInputScreen -> MenuConfirmScreen
 *
 * On completion, calls [onComplete] to pop back to the menu list.
 */
fun NavGraphBuilder.addMenuGraph(
    navController: NavController,
    onComplete: () -> Unit,
) {
    navigation(
        startDestination = ADD_MENU_SEARCH_ROUTE,
        route = ADD_MENU_GRAPH_ROUTE,
    ) {
        composable(route = ADD_MENU_SEARCH_ROUTE) { backStackEntry ->
            val parentEntry = remember(backStackEntry) {
                navController.getBackStackEntry(ADD_MENU_GRAPH_ROUTE)
            }
            val onboardingViewModel: OnboardingMenuViewModel = hiltViewModel(parentEntry)
        ...
        composable(
            route = ADD_MENU_DETAIL_ROUTE_PATTERN,
            arguments = listOf(
            ...
            MenuDetailScreen(
                ...
                onNavigateToIngredientInput = { menuDetailData ->
                    ...
                    val route = buildString {
                        append("$ADD_INGREDIENT_INPUT_ROUTE/")
                        append(menuDetailData.menuName.encodeForNavigation())
                        append("/${menuDetailData.isTemplateApplied}")
                        val templateId = onboardingViewModel.currentMenuDraft.value?.templateId
                        if (templateId != null) {
                            append("?$ARG_TEMPLATE_ID=$templateId")
                        }
                    }
        ...
        composable(
            route = ADD_INGREDIENT_INPUT_ROUTE_PATTERN,
            arguments = listOf(
                navArgument(ARG_MENU_NAME) { type = NavType.StringType },
                navArgument(ARG_IS_TEMPLATE_APPLIED) { type = NavType.BoolType },
                navArgument(ARG_TEMPLATE_ID) {
            ...
            IngredientInputScreen(
                ...
                onNavigateToConfirm = { ingredients ->
                    onboardingViewModel.addIngredients(ingredients)
                    onboardingViewModel.completeCurrentMenu()
                    navController.navigate(ADD_MENU_CONFIRM_ROUTE)
                },
            )
        }

        composable(route = ADD_MENU_CONFIRM_ROUTE) { backStackEntry ->
            val parentEntry = remember(backStackEntry) {
                navController.getBackStackEntry(ADD_MENU_GRAPH_ROUTE)
            }
            val onboardingViewModel: OnboardingMenuViewModel = hiltViewModel(parentEntry)
